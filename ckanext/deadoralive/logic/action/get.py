...
def get_resources_to_check(context, data_dict):
    ...
    """
    toolkit.check_access("ckanext_deadoralive_get_resources_to_check",context, data_dict)
...
def _broken_links_by_organization(context, organization_list, all_results, package_search):

    # Get a list of the names of all the site's organizations.
    organizations = organization_list(context=context,data_dict={'all_fields':True})
    # Get a dict mapping resource IDs to link checker results.
    result_dicts = {}
    num_r=0
    for result in all_results():                               #Gets all the links checked
        assert result["resource_id"] not in result_dicts
        result_dicts[result["resource_id"]] = result
        if result["num_fails"]>=1:                             #Counts the broken links
           num_r+=1
    ...
    for organization in organizations :
      ...
      if organization["name"] is not None:
         if organization["name"] in orgs or "test" in organization["name"]: #Test if the org is in the filter
             org_F = True
      if org_F: #Enter if org is in filter config
        organization_report_item = {
            "name": organization["name"],
            "display_name": (organization.get("title") or organization.get("display_name")),
            "image_display_url": organization["image_display_url"],
            "description": organization["description"],
            "num_broken_links": 0,                                  # Its needed the initialization of this value
            #"packages": organization["dataset"],                   #Incompability of versions, old reference to datasets
        ...
        # for each dataset).
        datasets = package_search(data_dict={'rows': 10000000})     #Gets all the datasets, changed from the original code
        #data_dict={"fq": "organization:{name}".format(name=organization["name"])})  #This produces some problems, old code

        # Build the report dict for each of the organization's datasets.
        for dataset in datasets:
           org_T = dataset["organization"]
           if org_T["name"] == organization["name"]: #Correct organization
              resource_ids = [resource["id"] for resource in dataset["resources"]]
              broken_resource_ids = [resource_id for resource_id in resource_ids if _is_broken(result_dicts.get(resource_id))]
              ...
              if broken_resource_ids:  # Only report datasets with broken links.
                  ...
                  organization_report_item["datasets_with_broken_links"].append(dataset_report_item)
           if num_broken_links:
              organization_report_item["num_broken_links"] = num_broken_links
              organization_report_item["datasets_with_broken_links"].sort(key=lambda x: x["num_broken_links"], reverse=True)
...
def broken_links_by_organization(context, data_dict):
    ...
    """
    toolkit.check_access("ckanext_deadoralive_broken_links_by_organization",context, data_dict)
...
def broken_links_by_email(context, data_dict):
    ...
    """
    toolkit.check_access("ckanext_deadoralive_broken_links_by_email",context, data_dict)
    ...
    for dataset in _package_search(data_dict={'rows': 1000000}):
        org_V = False          #Org filter  verification
        org_D = dataset["organization"]
        if org_D["name"] is not None:
           if org_D["name"] in orgs or 'test' in org_D["name"]:#Test if the org name from dataset is in the filter
              org_V = True
        broken_resources = []
        if org_V:
           ...
           if broken_resources:
              broken_datasets.append(dict(name=dataset["name"], title=dataset["title"],num_broken_links=len(broken_resources),resources_with_broken_links=broken_resources,email=_get_email_for_dataset(dataset)))
    ...
    for email in emails:
        ...
        emails[email]["datasets_with_broken_links"].sort(key=lambda x: x["num_broken_links"], reverse=True)
    ...
    for item in report:
        ...
        if len(item["datasets_with_broken_links"]) == 1:
            ...
            url = pylons.config["ckan.site_url"] + toolkit.url_for(controller="dataset", action="read", id=broken_dataset["name"])
            body = body.format(title=broken_dataset["title"], url=url)

        else:
            subject = u"You have {n} datasets with broken links on {site}"
            subject = subject.format(n=len(item["datasets_with_broken_links"]),site=pylons.config["ckan.site_title"])
            body = u"These datasets have broken links:"
            for dataset in item["datasets_with_broken_links"]:
                url = pylons.config["ckan.site_url"] + toolkit.url_for(controller="dataset", action="read", id=dataset["name"])
                body += u"%0A%0A{title}%0A{url}".format(title=dataset["title"], url=url)

        item["mailto"] = u"mailto:{email}?subject={subject}&body={body}".format(email=item["email"], subject=subject, body=body)
